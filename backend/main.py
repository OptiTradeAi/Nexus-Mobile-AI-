                ...
                function pushSignal(s) {
                    ...
                    el.innerHTML = `
                      ...
                      <div class="muted" style="margin-top:4px; font-variant-numeric: tabular-nums;">
                        entrada: <strong>${entryTime}</strong> (próx. vela M5)
                        ${lead !== '' ? ` — lead: ${lead}s` : ''}
                      </div>
...
async def ws_stream(ws: WebSocket):
    ...
    try:
        while True:
            ...
            # Análise (history, current)
            try:
                signal = analyze_candles(CANDLES[:-1], candle)
            except TypeError:
                signal = analyze_candles(CANDLES)  # fallback caso sua versão aceite 1 arg

            if signal and isinstance(signal, dict) and signal.get("send"):
                pair_norm = _normalize_pair(candle.get("pair", "UNK"))
