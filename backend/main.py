...
async def ws_stream(ws: WebSocket):
    ...
    try:
        while True:
            # Recebe blobs (chunks) do client/extension
            data = await ws.receive_bytes()
            # Heurística placeholder para 'confidence'
            conf = fake_confidence(len(data))
            # if confidence above threshold => register signal
            if conf >= SIGNAL_THRESHOLD:
                ...
                try:
                    if ws.application_state == WebSocketState.CONNECTED:
                        await ws.send_text(json.dumps({"type":"signal", "pair":rec["pair"], "confidence":conf, "timestamp":rec["timestamp"]}))
                except:
                    pass
            # send ack
            await ws.send_text(json.dumps({"type":"ack", "size": len(data), "confidence": conf}))
    except WebSocketDisconnect:
        print("Cliente desconectado")

def fake_confidence(nbytes:int)->float:
    # placeholder: estimativa baseada no tamanho do chunk
