...
(async function(){
  ...
  // procura elemento do gráfico (prioriza canvas)
  ...
  function canvasToDataURL(canvas, maxW=MAX_WIDTH, quality=QUALITY) {
    ...
    try {
      ...
      console.warn('drawImage falhou (possível canvas tainted)', e);
      return null;
    }
  ...
  async function sendFramePOST(pair, b64data) {
    try {
      ...
      console.warn('Erro envio POST frame', e);
    }
  }
  ...
  async function switchToPair(targetPair) {
    ...
    console.warn('Não conseguiu trocar para par', targetPair);
    return false;
  }
  ...
  async function mainLoop(){
    if(!AUTO_SWITCH){
      ...
      while(true){
        ...
        if(!switched) {
          console.warn('Switch falhou; tentando capturar mesmo assim para', pair);
        }
